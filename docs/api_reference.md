...
| `nanpercentile(input, q, dim=None, keepdim=False, interpolation='linear')` | `percentile` ignoring NaN, as `nanquantile` is to `quantile`. |
| `interp(x, xp, fp, left=None, right=None, period=None)` | Piecewise linear interpolation of the samples `(xp, fp)` at `x`, which may be a tensor of any shape. `xp` must increase; each query is placed by binary search, so the cost is `n log m` rather than `n * m`. Outside the range the value is held at the end point, or at `left`/`right` when given -- `left` applies strictly below the first sample, so a query landing *on* it still returns its value. `period` wraps both queries and samples onto one period so the ends join up. A query on a sample returns that sample exactly, not to within a rounding of the slope. |
| `digitize(input, bins, right=False)` | Which bin each value falls in, as an index into the edges `bins`: `0` below every edge, `len(bins)` above them all. `right` moves which side of an edge counts as inside it. The edges may increase *or* decrease -- a decreasing sequence is answered by searching the reversed edges and counting back from the total, which is why this is not simply `searchsorted`, whose contract is an increasing sequence. |
| `nancumsum(input, dim=None)` | The running sum along `dim` treating NaN as zero, so a NaN contributes nothing rather than poisoning every total after it. With no `dim` the tensor is flattened first. |
