...
1. **dtype dispatch macro for the ops layer** — the accessor layer is done;
   the same approach should collapse the per-dtype kernel copies in
   `operations/` (large LOC win, mechanical).
2. **Replace `include!` layout with real modules** — mechanical, improves
   tooling and visibility control (also re-enables
   `items_after_test_module`).
3. **Delete or feature-gate speculative subsystems** (`hardware`, `fusion`,
