...
class Tensor:
    ...
    def __init__(self, data: Any, requires_grad: bool = False, dtype: Optional[str] = None, device=None):
        """
        Initialize a tensor.
    ...
    # Gradient operations
    def backward(self, gradient: Optional['Tensor'] = None, retain_graph: bool = False, create_graph: bool = False):
    ...
    @staticmethod
    def zeros(*shape: Union[int, Sequence[int]], dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor filled with zeros."""
        if len(shape) == 1 and isinstance(shape[0], (list, tuple)):
            shape = shape[0]
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.zeros(list(shape), dtype, device, requires_grad)
        return result
    
    @staticmethod
    def ones(*shape: Union[int, Sequence[int]], dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor filled with ones."""
        if len(shape) == 1 and isinstance(shape[0], (list, tuple)):
            shape = shape[0]
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.ones(list(shape), dtype, device, requires_grad)
        return result
    
    @staticmethod
    def full(shape: Sequence[int], fill_value: float, dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor filled with a specific value."""
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.full(list(shape), fill_value, dtype, device, requires_grad)
        return result
    
    @staticmethod
    def rand(*shape: Union[int, Sequence[int]], dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor with random values from uniform distribution [0, 1)."""
        if len(shape) == 1 and isinstance(shape[0], (list, tuple)):
            shape = shape[0]
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.rand(list(shape), dtype, device, requires_grad)
        return result
    
    @staticmethod
    def randn(*shape: Union[int, Sequence[int]], dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor with random values from standard normal distribution."""
        if len(shape) == 1 and isinstance(shape[0], (list, tuple)):
            shape = shape[0]
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.randn(list(shape), dtype, device, requires_grad)
        return result
    
    @staticmethod
    def eye(n: int, m: Optional[int] = None, dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
    ...
    @staticmethod
    def arange(start: float, end: Optional[float] = None, step: float = 1.0, dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
        """Create a tensor with evenly spaced values."""
        if end is None:
            end = start
            start = 0.0
        result = Tensor.__new__(Tensor)
        result._tensor = _minitensor_core.Tensor.arange(start, end, step, dtype, device, requires_grad)
        return result
    
    @staticmethod
    def linspace(start: float, end: float, steps: int, dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
    ...
    @staticmethod
    def logspace(start: float, end: float, steps: int, base: float = 10.0, dtype: str = 'float32', device=None, requires_grad: bool = False) -> 'Tensor':
...
def tensor(data: Any, dtype: Optional[str] = None, device=None, requires_grad: bool = False) -> Tensor:
...
def full(shape: Sequence[int], fill_value: float, dtype: str = 'float32', device=None, requires_grad: bool = False) -> Tensor:
    """Create a tensor filled with a specific value."""
    return Tensor.full(shape, fill_value, dtype=dtype, device=device, requires_grad=requires_grad)
...
def arange(start: float, end: Optional[float] = None, step: float = 1.0, dtype: str = 'float32', device=None, requires_grad: bool = False) -> Tensor:
    """Create a tensor with evenly spaced values."""
    return Tensor.arange(start, end, step, dtype=dtype, device=device, requires_grad=requires_grad)

def linspace(start: float, end: float, steps: int, dtype: str = 'float32', device=None, requires_grad: bool = False) -> Tensor:
    """Create a tensor with linearly spaced values."""
    return Tensor.linspace(start, end, steps, dtype=dtype, device=device, requires_grad=requires_grad)
...
__all__ = [
    ...
    'eye', 'arange', 'linspace', 'from_numpy'
]