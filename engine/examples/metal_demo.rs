...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Max Threads Per Threadgroup: {}", metal_device.max_threads_per_threadgroup().width);
    ...
    let a_buffer = backend.create_buffer_with_data(&a_data, metal::MTLResourceOptions::StorageModeShared)?;
    let b_buffer = backend.create_buffer_with_data(&b_data, metal::MTLResourceOptions::StorageModeShared)?;
    ...
    println!("Matrix A (2x2): [{}, {}]", matrix_a[0..2].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
    println!("                [{}, {}]", matrix_a[2..4].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
    println!("Matrix B (2x2): [{}, {}]", matrix_b[0..2].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
    println!("                [{}, {}]", matrix_b[2..4].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));

    let a_mat_buffer = backend.create_buffer_with_data(&matrix_a, metal::MTLResourceOptions::StorageModeShared)?;
    let b_mat_buffer = backend.create_buffer_with_data(&matrix_b, metal::MTLResourceOptions::StorageModeShared)?;
    ...
    println!("Result (2x2):   [{}, {}]", mat_result[0..2].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
    println!("                [{}, {}]", mat_result[2..4].iter().map(|x| x.to_string()).collect::<Vec<_>>().join(", "));
    ...
    // ReLU
    let relu_input_buffer = backend.create_buffer_with_data(&activation_input, metal::MTLResourceOptions::StorageModeShared)?;
    let relu_output_buffer = backend.create_buffer(20, metal::MTLResourceOptions::StorageModeShared)?;
    ...
    // Sigmoid
    let sigmoid_input_buffer = backend.create_buffer_with_data(&activation_input, metal::MTLResourceOptions::StorageModeShared)?;
    let sigmoid_output_buffer = backend.create_buffer(20, metal::MTLResourceOptions::StorageModeShared)?;
    ...
    println!("Sigmoid: {:?}", sigmoid_result.iter().map(|x| format!("{:.4}", x)).collect::<Vec<_>>());
    ...
    if let Some(pipeline) = backend.get_compute_pipeline("add_kernel") {
        ...
        println!("Optimal 1D thread group size: {}x{}x{}", 
                 thread_group_size.width, thread_group_size.height, thread_group_size.depth);
        println!("Optimal 2D thread group size: {}x{}x{}", 
