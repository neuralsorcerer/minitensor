...
use crate::{
    ...
    error::Result,
    operations::{arithmetic, activation},
};
...
pub fn create_swish_op() -> Result<Arc<dyn CustomOp>> {
    CustomOpBuilder::new("swish", 1)
        ...
        .backward(|_grad_output, input_ids, input_shapes, input_dtypes, input_devices| {
        ...
        .validate(|inputs| {
            if inputs[0].numel() == 0 {
                return Err(MinitensorError::invalid_argument("Input tensor cannot be empty"));
            }
            Ok(())
        })
        .build()
...
pub fn create_mish_op() -> Result<Arc<dyn CustomOp>> {
    CustomOpBuilder::new("mish", 1)
        ...
        .backward(|grad_output, input_ids, input_shapes, input_dtypes, input_devices| {
            ...
            if let (Some(&input_id), Some(_input_shape), Some(&_input_dtype), Some(_input_device)) = (
                input_ids.get(0),
                input_shapes.get(0),
                input_dtypes.get(0),
                input_devices.get(0),
...
mod tests {
    ...
    fn test_power_op() {
        ...
        // Test validation
        let tensor1 = Tensor::ones(Shape::new(vec![2, 2]), DataType::Float32, Device::cpu(), false);
        let tensor2 = Tensor::ones(Shape::new(vec![3, 3]), DataType::Float32, Device::cpu(), false);
