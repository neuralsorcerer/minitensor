...
impl CudaBackend {
    ...
    pub fn load_kernel(&self, name: &str, ptx_src: &str) -> Result<()> {
        ...
        self.cuda_device
            .load_ptx(ptx, name, &[name])
            .map_err(|e| crate::error::MinitensorError::backend_error(format!("Failed to load CUDA kernel: {}", e)))?;

        let function = self.cuda_device
            .get_func(name, name)
            .map_err(|e| crate::error::MinitensorError::backend_error(format!("Failed to get CUDA function: {}", e)))?;
        ...
        T: cudarc::driver::DeviceRepr,
    {
        self.cuda_device
            .alloc_zeros::<T>(len)
            .map_err(|e| crate::error::MinitensorError::memory_error(format!("CUDA allocation failed: {}", e)))
    }
        ...
        T: cudarc::driver::DeviceRepr + Clone,
    {
        self.cuda_device
            ...
            .map_err(|e| crate::error::MinitensorError::memory_error(format!("Host to device copy failed: {}", e)))
    }
        ...
        T: cudarc::driver::DeviceRepr + Clone,
    {
        self.cuda_device
            ...
            .map_err(|e| crate::error::MinitensorError::memory_error(format!("Device to host copy failed: {}", e)))
    }
    ...
    pub fn synchronize(&self) -> Result<()> {
        self.cuda_device
            .synchronize()
            .map_err(|e| crate::error::MinitensorError::backend_error(format!("CUDA synchronization failed: {}", e)))
    }
}
...
impl Backend for CudaBackend {
    ...
    fn initialize() -> Result<Self> {
        ...
        let cuda_device = CudaDevice::new(device_id)
            .map_err(|e| crate::error::MinitensorError::backend_error(format!("Failed to initialize CUDA device: {}", e)))?;

        let cuda_device = Arc::new(cuda_device);
        let execution_context = Arc::new(Mutex::new(CudaExecutionContext::new(cuda_device.clone(), 8)));
...
impl CudaOps {
    ...
    pub fn new(backend: Arc<CudaBackend>) -> Result<Self> {
        ...
        ops.backend.load_kernel("matmul_kernel", kernels::MATMUL_KERNEL)?;
        ops.backend.load_kernel("relu_kernel", kernels::RELU_KERNEL)?;
        ops.backend.load_kernel("sigmoid_kernel", kernels::SIGMOID_KERNEL)?;
        
        Ok(ops)
    }

    /// Element-wise addition on GPU
    pub fn add(&self, a: &CudaSlice<f32>, b: &CudaSlice<f32>, c: &mut CudaSlice<f32>) -> Result<()> {
        let n = a.len();
        if b.len() != n || c.len() != n {
        ...
        unsafe {
            ...
            ).map_err(|e| crate::error::MinitensorError::backend_error(format!("Kernel launch failed: {}", e)))?;
        }
    ...
    pub fn mul(&self, a: &CudaSlice<f32>, b: &CudaSlice<f32>, c: &mut CudaSlice<f32>) -> Result<()> {
        let n = a.len();
        if b.len() != n || c.len() != n {
            return Err(crate::error::MinitensorError::shape_error(
    ...
    pub fn matmul(&self, a: &CudaSlice<f32>, b: &CudaSlice<f32>, c: &mut CudaSlice<f32>, m: usize, n: usize, k: usize) -> Result<()> {
