...
pub trait Module: Layer {
    ...
    fn load_state_dict(&mut self, state_dict: &crate::serialization::StateDict, device: Option<crate::device::Device>) -> Result<()> {
        // Load parameters
        ...
        if named_params.is_empty() {
            ...
            for (i, param_ref) in params.iter_mut().enumerate() {
                if let Ok(loaded_tensor) = state_dict.load_parameter(&format!("param_{}", i), device) {
