...
impl Conv2d {
    ...
    pub fn new(
        ...
        let weight_shape = Shape::new(vec![out_channels, in_channels, kernel_size.0, kernel_size.1]);
...
impl Layer for Conv2d {
    fn forward(&mut self, input: &Tensor) -> Result<Tensor> {
        ...
        let output_shape = Shape::new(vec![batch_size, self.out_channels, output_height, output_width]);
                ...
                let bias_data = if let Some(ref bias) = self.bias {
                    Some(
                        bias
                            .data()
                            .as_f32_slice()
                            .ok_or_else(|| MinitensorError::invalid_operation("Expected f32 bias data"))?,
                    )
                } else {
                    None
                };

                let mut output_vec = vec![0f32; batch_size * self.out_channels * output_height * output_width];
                                    ...
                                    for kh in 0..self.kernel_size.0 {
                                        for kw in 0..self.kernel_size.1 {
                                            ...
                                            if ih < input_height && iw < input_width {
                                                let input_idx = ((n * self.in_channels + ic) * input_height + ih)
                                                    * input_width
                                                    + iw;
                                                let weight_idx =
                                                    ((oc * self.in_channels + ic) * self.kernel_size.0 + kh)
                                                        * self.kernel_size.1
                                                        + kw;
...
mod tests {
    use super::*;
    use crate::tensor::{Tensor, TensorData, Shape, DataType};
    ...
    fn test_conv2d_forward_shape_validation() {
        ...
        let input = Tensor::zeros(Shape::new(vec![2, 3, 32, 32]), DataType::Float32, Device::cpu(), false);
        let output = layer.forward(&input).unwrap();
        assert_eq!(output.shape(), &Shape::new(vec![2, 16, 32, 32]));
        
        // Test with incorrect number of channels
        let wrong_input = Tensor::zeros(Shape::new(vec![2, 5, 32, 32]), DataType::Float32, Device::cpu(), false);
        let result = layer.forward(&wrong_input);
        assert!(result.is_err());
        
        // Test with wrong number of dimensions
        let wrong_dim_input = Tensor::zeros(Shape::new(vec![2, 3, 32]), DataType::Float32, Device::cpu(), false);
    ...
    fn test_conv2d_forward_computation() {
        ...
        let input = Tensor::new(
            Arc::new(TensorData::from_vec_f32(vec![1.0, 2.0, 3.0, 4.0], Device::cpu())),
