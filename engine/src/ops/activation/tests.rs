...
//! Tests for the activation ops, which live in the sibling modules
//! (`elementwise`, `power`, `softmax`, `trigonometry`, `hyperbolic`).
//!
//! This file used to carry one implementation as well -- `ceil_f64`, alone,
//! while `ceil_f32` and the rest of the rounding family sat in `power.rs`. It
//! has moved to join them, which is also what let the whole family become one
//! multiversioned kernel.
...
fn test_nan_to_num_backward_masks_replaced_entries() {
    ...
    let summed = crate::ops::reduction::sum(&result, None, false).unwrap();
...
fn test_masked_softmax_all_negative_infinity_unmasked() {
    let tensor = create_test_tensor_f32(vec![f32::NEG_INFINITY, f32::NEG_INFINITY], vec![2], false);
