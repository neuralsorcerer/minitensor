...
use super::optimizer::{Optimizer, ParameterGroup, GradientClipping};
...
impl SGD {
    ...
    fn apply_weight_decay(&self, _param: &Tensor, grad: &Tensor, _weight_decay: f64) -> Result<Tensor> {
    ...
    fn apply_simple_update(&mut self, param: &mut Tensor, grad: &Tensor, _lr: f64) -> Result<()> {
        ...
        if param.device() != grad.device() {
            return Err(crate::error::MinitensorError::device_mismatch(
                param.device().to_string(), grad.device().to_string()
            ));
        }
