...
#[cfg(test)]
mod tests {
    use crate::optim::{SGD, Adam, RMSprop, ParameterGroup, Optimizer};
    ...
    fn test_optimizer_zero_grad() {
        ...
        let mut tensor1 = Tensor::zeros(Shape::new(vec![2, 2]), DataType::Float32, Device::cpu(), true);
        let mut tensor2 = Tensor::zeros(Shape::new(vec![3, 3]), DataType::Float32, Device::cpu(), true);
        
        // Set some gradients
        let grad1 = Tensor::ones(Shape::new(vec![2, 2]), DataType::Float32, Device::cpu(), false);
        let grad2 = Tensor::ones(Shape::new(vec![3, 3]), DataType::Float32, Device::cpu(), false);
    ...
    fn test_optimizer_step_not_implemented() {
        ...
        let mut tensor = Tensor::zeros(Shape::new(vec![2, 2]), DataType::Float32, Device::cpu(), true);
        
        // Set a gradient
        let grad = Tensor::ones(Shape::new(vec![2, 2]), DataType::Float32, Device::cpu(), false);
