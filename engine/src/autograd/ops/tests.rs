...
mod tests {
    ...
    use crate::ops::{activation, arithmetic, reduction};
    ...
    fn test_matmul_backward_gradients() {
        ...
        let grads = grad_fn.backward(&grad_output).unwrap();
        let rhs_t = crate::ops::linalg::transpose(&rhs, 0, 1).unwrap();
        let expected_lhs = crate::ops::linalg::matmul(&grad_output, &rhs_t).unwrap();
    ...
    fn test_matmul_backward_batched() {
        ...
        .unwrap();
        let expected_rhs = crate::ops::linalg::matmul(&lhs_t, &grad_output).unwrap();
        assert!(
            grads
                .get(&input_ids[1])
                .unwrap()
    ...
    fn test_transpose_backward_permutation() {
        ...
        for i in 0..inverse.len() {
            ...
            if i != j {
                expected =
                    crate::ops::linalg::transpose(&expected, i as isize, j as isize).unwrap();
                current.swap(i, j);
            }
        }
        assert!(grad_input.allclose(&expected, 1e-6, 1e-6));
    ...
    fn test_loss_targets_receive_no_gradient() {
        ...
        let loss = crate::ops::loss::mse_loss(&predictions, &targets, "mean").unwrap();
        ...
        let loss = crate::ops::loss::mse_loss(&predictions, &targets_rg, "mean").unwrap();
        let grads = backward_collect(&loss, None).unwrap();
    ...
    fn test_cross_entropy_records_only_its_analytical_node() {
        ...
        let loss = crate::ops::loss::cross_entropy_loss(&logits, &targets, "mean").unwrap();
    ...
    fn test_concat_frozen_inputs_receive_no_gradient() {
        ...
        let out = crate::ops::shape_ops::concatenate(&[&trainable, &frozen], 0).unwrap();
        let grad = Tensor::ones(out.shape().clone(), out.dtype(), out.device(), false);
        let grads = backward_collect(&out, Some(grad)).unwrap();
