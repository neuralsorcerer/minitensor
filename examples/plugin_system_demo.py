...
def create_math_operations():
    ...
    def square_backward(
        ...
        # For square operation: d/dx(x^2) = 2x
        # This is a simplified implementation
        gradients = {}
        if len(input_ids) > 0:
            # In a real implementation, we'd need access to the original input
            # For demo purposes, we'll just return the gradient output scaled by 2
    ...
    square_op = (
        ...
        .backward(square_backward)
        .validate(
            lambda inputs: None if len(inputs) == 1 else "Square requires one input"
        )
        .output_shape(lambda shapes: shapes[0])
        .output_dtype(lambda dtypes: dtypes[0])
        .output_device(lambda devices: devices[0])
        .build()
    )

    # Cube operation
    def cube_forward(inputs):
        x = inputs[0]
        return x * x * x
    ...
    def cube_backward(
        ...
        # For cube operation: d/dx(x^3) = 3x^2
        gradients = {}
        if len(input_ids) > 0:
            # Simplified implementation
            gradients[input_ids[0]] = grad_output * 3
        return gradients

    cube_op = (
        mt.CustomOpBuilder("cube", 1)
        .forward(cube_forward)
        .backward(cube_backward)
        .validate(
            lambda inputs: None if len(inputs) == 1 else "Cube requires one input"
        )
        .output_shape(lambda shapes: shapes[0])
        .output_dtype(lambda dtypes: dtypes[0])
        .output_device(lambda devices: devices[0])
...
class CustomNormalizationLayer(plugins.CustomLayer):
    ...
    def __init__(self, normalized_shape, eps=1e-5):
        super().__init__("custom_layer_norm")
    ...
    def forward(self, inputs):
        ...
        mean = mt.mean(x, dim=-1, keepdim=True)
        var = mt.var(x, dim=-1, keepdim=True)

        # Normalize
        x_norm = (x - mean) / mt.sqrt(var + self.eps)
...
def demo_custom_operations():
    ...
    # Register custom operations globally
    operations = create_math_operations()
    for op in operations:
        mt.register_custom_op(op)

    # List registered operations
    print("Registered custom operations:")
    for op_name in mt.list_custom_ops():
        print(f"  - {op_name}")
    ...
    if mt.is_custom_op_registered("square"):
        y_square = mt.execute_custom_op("square", [x])
        print(f"Square result: {y_square}")

        # Test backward pass
        loss = mt.sum(y_square)
        loss.backward()
        print(f"Gradient after square: {x.grad}")

    # Reset gradients
    x.grad = None

    # Test cube operation
    if mt.is_custom_op_registered("cube"):
        y_cube = mt.execute_custom_op("cube", [x])
        print(f"Cube result: {y_cube}")

        # Test backward pass
        loss = mt.sum(y_cube)
        loss.backward()
        print(f"Gradient after cube: {x.grad}")

    # Clean up
    mt.unregister_custom_op("square")
    mt.unregister_custom_op("cube")
...
def demo_custom_layers():
    ...
    try:
        ...
        mean = mt.mean(output, dim=-1)
        var = mt.var(output, dim=-1)
